...
import { Calculator, FileText, Users, TrendingUp, Shield, Smartphone, Check, ArrowRight, BarChart3, Receipt, Wallet, Building2 } from 'lucide-react';
const features = [{
  icon: Calculator,
...
const plans = [{
  ...
  features: ['1 организация', '100 операций в месяц', '2 пользователя', 'Базовые отчёты'],
  popular: false
}, {
  name: 'Бизнес',
  price: '9 990',
  description: 'Для малого бизнеса',
  features: ['5 организаций', 'Без ограничений операций', '10 пользователей', 'Все отчёты', 'Экспорт в PDF', 'Приоритетная поддержка'],
  popular: true
}, {
  name: 'Про',
  price: '29 990',
  description: 'Для растущих компаний',
  features: ['Без ограничений организаций', 'Без ограничений операций', 'Без ограничений пользователей', 'API интеграции', 'Брендирование документов', 'Персональный менеджер'],
  popular: false
}];
const faqs = [{
  question: 'Что такое БухОнлайн?',
  answer: 'БухОнлайн — это облачный сервис для ведения финансового учёта малого бизнеса. Вы можете учитывать доходы и расходы, выставлять счета, вести справочники контрагентов и получать отчёты.'
}, {
  question: 'Подходит ли сервис для ИП?',
  answer: 'Да, сервис подходит для индивидуальных предпринимателей, ТОО и других форм бизнеса в Казахстане и СНГ.'
}, {
  question: 'Как импортировать данные из другой системы?',
  answer: 'Вы можете импортировать операции через CSV-файл. Скачайте шаблон, заполните его и загрузите в систему.'
}, {
  question: 'Можно ли работать с несколькими организациями?',
  answer: 'Да, в тарифах Бизнес и Про вы можете вести учёт нескольких организаций и переключаться между ними.'
}, {
  question: 'Как пригласить бухгалтера?',
  answer: 'В настройках организации отправьте приглашение по email. Бухгалтер получит доступ к учёту без возможности управлять тарифом.'
}];
      ...
      <header className="sticky top-0 z-50 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container flex h-16 items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xl font-bold">​Азамат Accounting </span>
          </div>
            ...
            <div className="relative rounded-xl border bg-card p-2 shadow-lg">
              ...
              <div className="relative rounded-lg bg-muted/30 p-4 md:p-8">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                  ...
                  icon: FileText,
                  positive: true
                }].map((stat, i) => <div key={i} className="rounded-lg bg-card p-4 shadow-sm">
      ...
      <section id="features" className="py-20 bg-muted/30">
        <div className="container">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {features.map((feature, i) => <Card key={i} className="card-elevated hover:shadow-md transition-shadow">
                <CardHeader>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <ul className="mt-8 space-y-4">
                {['Быстрое добавление операций', 'Просмотр отчётов на ходу', 'Управление счетами', 'Работа офлайн с синхронизацией'].map((item, i) => <li key={i} className="flex items-center gap-3">
      ...
      <section id="pricing" className="py-20 bg-muted/30">
        <div className="container">
          ...
          <div className="grid gap-6 md:grid-cols-3 max-w-5xl mx-auto">
            {plans.map((plan, i) => <Card key={i} className={`relative ${plan.popular ? 'border-primary shadow-lg scale-105' : ''}`}>
