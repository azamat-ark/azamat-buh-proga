...
export default function JournalEntries() {
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      // Validate and filter lines with proper number validation
      const validatedLines: Array<{ account_id: string; debit: number; credit: number; description: string | null }> = [];
      
      for (const line of lines) {
        if (!line.account_id) continue;
        
        const debitValidation = validateNonNegativeNumber(line.debit);
        const creditValidation = validateNonNegativeNumber(line.credit);
        
        if (!debitValidation.valid) {
          throw new Error(`Неверное значение дебета: ${debitValidation.error}`);
        }
        if (!creditValidation.valid) {
          throw new Error(`Неверное значение кредита: ${creditValidation.error}`);
        }
        
        if (debitValidation.value === 0 && creditValidation.value === 0) continue;
        
        validatedLines.push({
          account_id: line.account_id,
          debit: debitValidation.value,
          credit: creditValidation.value,
          description: line.description?.slice(0, 500) || null,
        });
      }
      
      if (validatedLines.length < 2) {
      ...
      if (Math.abs(totalDebit - totalCredit) > 0.001) {
        throw new Error(`Дебет (${totalDebit.toFixed(2)}) не равен кредиту (${totalCredit.toFixed(2)})`);
      }

      // Create entry - entry_number is auto-generated by database trigger
      const tempEntryNumber = `TEMP-${Date.now()}`;
      const { data: entry, error: entryError } = await supabase
        .from('journal_entries')
        .insert([{
          company_id: currentCompany!.id,
          entry_number: tempEntryNumber, // Will be replaced by trigger
          date: format(formData.date, 'yyyy-MM-dd'),
          period_id: formData.period_id,
          document_type_id: formData.document_type_id || null,
          description: formData.description?.slice(0, 1000) || null,
          created_by: user?.id,
        }])
        .select()
        .single();
      ...
      // Create lines
      const linesToInsert = validatedLines.map((l, i) => ({
        entry_id: entry.id,
